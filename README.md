...
[![Desktop](https://img.shields.io/badge/Desktop-Tkinter%209.0-blue.svg)](#)
[![CLI-UI](https://img.shields.io/badge/UI-Rich-purple.svg)](#)

> **Aether-Link** is a high-level AI research assistant available as both a **modern Desktop GUI application** and an **interactive Terminal CLI**. Powered by Google's next-generation Gemini models via the `google-genai` SDK, it provides real-time streaming, dynamic persona switching with persistent memory, local file context injection, and session exportation.

---

## Overview

Aether-Link is engineered to assist developers, researchers, and writers directly within their native desktop or shell environments. With full feature parity between the Desktop App and CLI, you can switch seamlessly between visual and terminal workflows.

### Key Capabilities

- **Modern Desktop GUI**: Built with Python standard library's **Tkinter 9.0** featuring native macOS high-DPI dark mode, sidebar persona controls, file attachment pills, and thread-safe real-time streaming.
...
- **File & Code Context Injection**: Directly inject local file contents into your inquiries via the GUI "Attach File" picker, the `/file <path>` command, or inline `@filename` mentions.
- **Session Export**: Save full multi-turn conversation logs and metadata into formatted Markdown files via `/save [filename]` or the Desktop GUI export button.
...
| **Desktop GUI** | Tkinter 9.0 (`tkinter`, `ttk`) | Native macOS dark mode windowing, non-blocking worker threads |
...
*(Note: The Desktop GUI uses Python's built-in `tkinter`, requiring zero extra GUI package installations!)*
...
# Optional: override default model (default: gemini-3.7-flash)
...
### 🖥️ Option 1: Desktop Application (GUI)

Launch the desktop app using the master launcher or directly:

```bash
python3 main.py
# or
python3 aether_gui.py
```

#### Desktop GUI Features:
- **Left Sidebar**: One-click persona switcher (🔬 Research Assistant, 💻 Python Coder, ✍️ Creative Novelist).
- **Context Injection**: Click **📂 Attach File Context** to select any project file via the native macOS file picker.
...
- **Chat Window**: Real-time token streaming cards with live Markdown rendering, status indicator (🟢 Online / 🟣 Thinking...), elapsed time (⏱️), and token count metrics (🔢).
- **Input Field**: Type multi-line queries (Enter to send, Shift+Enter for newline). All slash commands and `@filename` mentions work seamlessly in the GUI input box!
...
#### Supported Commands in Desktop & CLI:
 
| Command / Action | Desktop Equivalent | Role & Description |
| :--- | :--- | :--- |
| `/coder` | Click **💻 Expert Python Programmer** | Switch persona to `"You are an expert Python programmer."` |
...
| `/file <path> [prompt]` | Click **📂 Attach File Context** | Injects local file content into the prompt context |
| `@<filepath>` | Type `@filename` in prompt | Automatically attaches local file mentioned anywhere in prompt |
| `/save [filename]` | Click **💾 Export Session** | Exports the current chat transcript to Markdown in `saved_sessions/` |
...
| `exit`, `quit`, `/exit` | Close Window | Cleanly terminates the session |
...
├── aether_gui.py      # Desktop GUI application (Tkinter 9.0)
├── aether_link.py     # Terminal assistant client (Rich CLI)
├── main.py            # Master launcher (Desktop GUI default, --cli for terminal)
├── saved_sessions/    # Default directory for exported Markdown sessions
